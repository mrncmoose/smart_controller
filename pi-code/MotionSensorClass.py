...
class MotionAction:
    def __init__(self, 
        ...
        if self.timeOutSeconds > 10:
            self.bounceTime_ms = 1500
        else:
            self.bounceTime_ms = 1000
        #GPIO.setup(self.sensorInPin, GPIO.IN, pull_up_down=GPIO.PUD_DOWN)
        GPIO.setup(self.sensorInPin, GPIO.IN, pull_up_down=GPIO.PUD_UP)
        GPIO.add_event_detect(self.sensorInPin, GPIO.RISING, callback=self.motionAction, bouncetime=self.bounceTime_ms)
        #GPIO.add_event_detect(self.sensorInPin, GPIO.BOTH, callback=self.motionAction, bouncetime=self.bounceTime_ms)
    ...
    def setTimerValue(self, timeOutSeconds:int):
        if self.timeOutSeconds != timeOutSeconds:
            ...
            self.logger.debug('Same timeout value sent as current timeout value.')
    ...
    def checkForTimeOut(self):
        ...
        self.logger.debug('{} seconds until time out.'.format(self.deltaTime))
